...
class RampSnippetManager {
  private snippets = new Map<string, RampSnippet[]>();
  private customSnippets = new Map<string, RampSnippet[]>();
  ...
  private loadBuiltInSnippets() {
    ...
    // Python Snippets
    const pythonSnippets: RampSnippet[] = [
      {
        name: "FastAPI Route",
        prefix: "fapi",
        body: [
    ...
    // Java Snippets
    const javaSnippets: RampSnippet[] = [
      {
    ...
    // HTML Snippets
    const htmlSnippets: RampSnippet[] = [
      {
  ...
  private initializeSnippetStrings() {
  ...
  private loadCustomSnippets() {
    // Load custom snippets from workspace settings or global storage
    const customSnippetsData = this.context.globalState.get<any>('ramp.customSnippets', {});
    
    Object.keys(customSnippetsData).forEach(language => {
      const snippets = customSnippetsData[language].map((data: any) => ({
        ...data,
  ...
  public getSnippetsForLanguage(language: string): RampSnippet[] {
    const builtIn = this.snippets.get(language) || [];
    const custom = this.customSnippets.get(language) || [];
    return [...builtIn, ...custom];
  }

  public async addCustomSnippet(language: string, snippet: Omit<RampSnippet, 'insertText'>) {
    const fullSnippet: RampSnippet = {
      ...snippet,
  ...
  public async removeCustomSnippet(language: string, snippetName: string) {
    const existing = this.customSnippets.get(language) || [];
  ...
  private async saveCustomSnippets() {
    const data: any = {};
  ...
  public async showSnippetManager() {
...
export function registerSnippetManager(context: vscode.ExtensionContext): RampSnippetManager {
