...
class RampCompletionProvider implements vscode.CompletionItemProvider {
  private cache: CompletionCache = {};
  private workspaceSymbols = new Map<string, SymbolInfo>();
  ...
  private loadLanguageDefinitions(language: string) {
  ...
  private getAdvancedLanguageDefinitions(language: string): SymbolInfo[] {
    const definitions: { [key: string]: SymbolInfo[] } = {
      javascript: [
        {
          name: "console.log",
  ...
  private getLanguagePatterns(language: string): PatternInfo[] {
  ...
  private extractSymbols(text: string, language: string): SymbolInfo[] {
  ...
  private shouldIncludeSymbol(symbol: SymbolInfo, currentWord: string, linePrefix: string): boolean {
  ...
  private getSortText(score: number): string {
  ...
  private createImportSuggestion(): vscode.CompletionItem {
  ...
  private createExportSuggestion(): vscode.CompletionItem {
  ...
  private createPythonImportSuggestion(): vscode.CompletionItem {
